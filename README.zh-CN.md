...
- 支持 JSON 字符串解析成 `URL`
- 支持 JSON 数字和数字字符串解析成 `Decimal`
- 支持使用 `@SafeEnum` 解析 raw-value 枚举
...
`@SafeEnum` 支持 `String`、`Int`、`Double`、`Float`、`Bool` raw value。非法枚举值会使用属性默认值。
